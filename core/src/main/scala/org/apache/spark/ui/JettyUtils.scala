...
private[spark] object JettyUtils extends Logging {
  ...
  def startJettyServer(
    ...
    var oldHandler: Option[ContextHandler] = None
    ...
    def connect(currentPort: Int): ((Server, Option[Int]), Int) = {
      ...
      val httpsConnector = sslOptions.createJettySslContextFactory() match {
        case Some(factory) =>
          ...
          oldHandler.foreach(collection.removeHandler(_))
          val handler = createRedirectHttpsHandler(securePort, scheme)
          oldHandler = Some(handler)

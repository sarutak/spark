...
private[client] class Shim_v2_0 extends Shim with Logging {
  ...
  private lazy val dropIndexMethod =
    findMethod(
      ...
      JBoolean.TYPE,
      JBoolean.TYPE)
  private lazy val getIndexMethod =
    findMethod(
      classOf[Hive],
      "getIndex",
      classOf[String],
  ...
  override def dropIndex(hive: Hive, dbName: String, tableName: String, indexName: String): Unit = {
    ...
    dropIndexMethod.invoke(
      dbName, tableName, indexName, throwExceptionInDropIndex, deleteDataInDropIndex)
  }
  ...
  override def renamePartition(
      ...
      newPart: Partition): Unit = {
    recordHiveCall()
    renamePartitionMethod.invoke(table, oldPartSpec, newPart)
  }
  ...
  override def getIndexes(
      ...
      max: Short): Seq[Any] = {
    recordHiveCall()
    getIndexMethod.invoke(dbName, tableName, max).asInstanceOf[JList[Any]].asScala.toSeq
  }
}

...
object SQLConf {
  ...
  val MERGE_GROUPING_AGGREGATES_ENABLED =
    buildConf("spark.sql.optimizer.mergeGroupingAggregates.enabled")
      .doc("When set to true, grouping aggregates with identical GROUP BY keys and the same " +
        "child plan are merged into a single aggregate stored as a CTE, eliminating " +
